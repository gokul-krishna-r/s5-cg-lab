...
def drawScene():
   ...
   drawCircle(ROTATEX+300,ROTATEY+300,10)
   glFlush()
   glutSwapBuffers()
...
def boundaryFill(x,y,bColor,newColor):
   color=glReadPixels(x,y,1.0,1.0,GL_RGB,GL_FLOAT,None)
   print(color[0][0],x,y)
   if((color[0][0][0]!=newColor[0] or color[0][0][1]!=newColor[1] or color[0][0][2]!=newColor[2] ) and
        (color[0][0][0]!=bColor[0] or color[0][0][1]!=bColor[1] or color[0][0][2]!=bColor[2] )):
      glColor3f(*newColor)
      glPointSize(2)
      glBegin(GL_POINTS)
      glVertex2i(x,y)
      glEnd()
      boundaryFill(x+2,y,bColor,newColor)
      boundaryFill(x-2,y,bColor,newColor)
      boundaryFill(x,y+2,bColor,newColor)
      boundaryFill(x,y-2,bColor,newColor)
   glFlush()
